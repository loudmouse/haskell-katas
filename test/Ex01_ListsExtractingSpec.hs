...
spec = do
  describe "Extracting Portion of List" $ do
    ...
    it "finds the tail part of a list" $ tail [1, 2, 3, 4, 5] `shouldBe` [2, 3, 4, 5]
    it "finds the last element in a list" $ last [1,2,3,4,5] `shouldBe` 5
    it "extracts the elements except the last one from a list" $ init [1, 2, 3, 4, 5] `shouldBe` [1, 2, 3, 4]
    ...
    it "can take with a while" $ do
      ...
      takeWhile (< 'b') "abracadabra" `shouldBe` "a"
    it "can drop while" $ do
      dropWhile (< 5) [1 .. 10] `shouldBe` [5, 6, 7, 8, 9, 10]
      dropWhile (< 'b') "abracadabra" `shouldBe` "bracadabra"
